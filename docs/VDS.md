...
| 項目 | 内容 |
|------|------|
| エンドポイント | `POST /synth`（単一 cue / ドラマモード）、`POST /synth/vds`（テキストファイルアップロード） |
...
上流 [Irodori-TTS](https://github.com/Aratako/Irodori-TTS) は LoRA 指定に加えて、自然文による話者記述（**caption**、例: 「若い女性、落ち着いた声」）で話者特性を条件付けできる。`POST /synth` は `caption` フィールドをサポートしており、サーバに `caption_hf_repo` が構成されていれば VoiceDesign 経路で合成される。未構成の場合は HTTP 501 を返す。
...
| ディレクティブ | 意味 | 例 |
|----------------|------|----|
| `@version: <int>` | フォーマット版。**必須**。省略するとパースエラー | `@version: 1` |
...
| `@defaults <key>=<value>, ...` | 全cueに適用するサンプリングパラメータの既定値。cue側のオプションが優先。`gap` キーで cue 間デフォルト無音（秒）を指定可能（後述 §3.6） | `@defaults num_steps=50, gap=1.0` |
| `@scene: <text>` | シーン区切り。パーサが認識しメタデータとして保持するが、v1 では合成・再生に影響しない。cue の前後どこにでも配置可能 | `@scene: 第1幕 朝の会話` |
...
- `@scene` は cue の前後**どこにでも**配置可能（シーン区切りのため）。
- 同じエイリアスの再定義はエラー。
...
### 3.6 無音（pause）とデフォルトギャップ

**デフォルトギャップ:**

連続する speech cue の間には、暗黙の無音（**gap**）が挿入される。

- `@defaults gap=<seconds>` で指定する。**未指定時のデフォルトは `1.0` 秒**。
- `gap=0` を指定すると暗黙の無音は挿入されない。
- gap は **speech → speech** の遷移にのみ適用される。speech と pause の間には gap は挿入されない（pause が明示的な間を制御するため）。
...
- speech cue 間に明示的な `(pause)` がある場合、その区間のデフォルト gap は**置換**される（加算されない）。
- 連続する `(pause)` は**加算**される（例: `(pause 1.0)` + `(pause 2.0)` = 3.0秒の無音）。ただし意図しない重複の可能性があるため**警告**を出す。
- 先頭・末尾の `(pause)` も許容するが、実質的に再生前後の無音となる。
...
- 実行時：
  ...
  3. 定義が **caption 形式** の場合、`caption` フィールドを伴う合成経路（VoiceDesignモード）に投げる。サーバに `caption_hf_repo` が構成されていない場合は HTTP 501 となる（§6.3）。
...
@scene: 朝の挨拶
chieri: おはよう、つむぎ。
# ↑↓ speech→speech なのでデフォルト gap（0.3秒）の無音が入る
young_woman: おはようございます、ちえりさん。

(pause 0.8)
# ↑ 明示的 pause があるので gap は置換される（0.8秒の無音）
...
type SpeakerRef =
  | { type: "lora"; uuid: string }        // LoRA（既存の /synth 経路）
  | { type: "caption"; caption: string }   // 自然文記述（VoiceDesign 経路、API 未対応）
...
type Cue =
  ...
  | { kind: 'scene'; name: string }    // シーン区切り（v1 では合成・再生に影響しない）
```
...
3. `kind = "scene"` の場合:
   - v1 では**スキップ**（合成・再生に影響しない）。メタデータとして保持のみ。
   - 将来的にシーン単位のエクスポート・部分再生に利用する。

**出力形式:**

HTTP `Accept` ヘッダによる 2 形式を提供する:

- **`Accept: audio/pcm`**（デフォルト）: Raw PCM16 mono（signed int16 LE, 24000 Hz）のストリーミング。各 cue の合成完了時に即座に yield するため低レイテンシ。`X-TTS-Sample-Rate` ヘッダでサンプルレートを通知。
- **`Accept: audio/wav`**: 全 cue の合成完了後に結合した単一 WAV ファイルを返却。

pause / gap は指定秒数の無音 PCM として挿入される。

**gap の挿入ロジック:**

cue リストを順に処理する際、直前の cue が `kind = "speech"` で、現在の cue も `kind = "speech"` であれば、その間に `defaults.gap`（未指定時 `1.0` 秒）の無音を挿入する。間に `kind = "pause"` が挟まっている場合、gap は挿入しない（pause が間を制御する）。
...
- 連続する `(pause)` が重なっている（加算はされるが、意図しない重複の可能性）
- `speakers` に未使用のエイリアス
- `defaults` に未知キー（将来互換のため警告止まり）
- speech cue 数が **100** を超えている（推奨上限。実装側で制限を強制するかは任意）

### 6.3 実行時エラー

- UUID エイリアスの UUID が `GET /speakers` に存在しない → `/synth` が 404 を返した時点で該当 cue をスキップし、再生パイプライン全体は継続（設計選択、変更可）。**スキップされた cue の前後の `(pause)` はそのまま維持**される（除去・結合は行わない）。
- **caption エイリアスを含む cue を、caption 未構成のサーバに投入した場合** → HTTP 501。ドラマモードでは事前検証で検出。前後の pause は維持。
...
- **日本語エイリアス**: エイリアスは ASCII のみ（`[A-Za-z_][A-Za-z0-9_-]*`）。Unicode 識別子を許容すると、漢字・ひらがな・カタカナの混在や全角半角の正規化問題が生じ、パーサの境界判定が複雑化する。v2 で Unicode Letter カテゴリの許容を検討する。
- ~~**単一結合 WAV エクスポート**: `Accept: audio/wav` で対応済み。~~
...
- **シーン単位の制御**: `@scene` は v1 でメタデータとして導入済みだが、シーン単位のエクスポート・部分再生・シーンごとの defaults 上書きは v2 で検討
- **プリセット**: `@preset emotional = cfg_scale_text=4.0, ...` のような名前付きオプション束
- ~~**Annotation（shortcode）**: v1 で実装済み。39 種のビルトイン shortcode をパース時に展開する。詳細は [VDS_SPEC_V1.md §4](VDS_SPEC_V1.md#4-annotationshortcode) を参照。~~
...
以下は v1 仕様のブラッシュアップ候補。対応済みになったらチェックを入れる。

### 高優先

- [x] **cue間デフォルト無音の定義**: `@defaults gap=<seconds>` で制御。未指定時のデフォルトは `1.0` 秒。speech→speech 間にのみ適用、明示的 `(pause)` がある区間では置換される（§3.6）
- [x] **連続 `(pause)` の挙動明確化**: 連続する pause は**加算**される。警告は出すが処理は継続（§3.6, §6.2）
- [x] **最終出力形式の定義**: ストリーミング再生（cue 単位で逐次 enqueueAudio に投入）。単一結合 WAV は将来拡張（§5）
- [x] **`@version` の必須化**: 初版から必須。省略時はパースエラー（§3.4, §6.1）

### 中優先

- [x] **空テキストの扱い**: `alias:` のみ（テキストなし）は致命的エラー（§6.1）
- [x] **cueスキップ時のpause挙動**: スキップされた cue の前後の pause はそのまま維持（§6.3）
- [x] **JSON `SpeakerRef` に判別子追加**: `type: "lora" | "caption"` を必須フィールドとして追加（§4.1, §4.2）
- [x] **スクリプト長の上限**: speech cue 数の推奨上限を **100** と定義（§6.2 警告）。実装側で制限を強制するかは任意

### 低優先（v2 検討枠）

- [x] **日本語エイリアス対応**: v1 では ASCII のみ（`[A-Za-z_][A-Za-z0-9_-]*`）に制限。設計判断を §8 に記載済み
- [x] **`@scene` セクション区切り**: v1 でメタデータとして導入（合成・再生には影響しない）。シーン単位の制御は v2（§3.4, §4.1, §5, §8）
...
| 1.1 | 2026-04-23 | TODO 全10件を解決。主な変更：`@defaults gap` によるcue間デフォルト無音（1.0秒）、連続pause加算、ストリーミング再生出力、`@version` 必須化、空テキストエラー化、cueスキップ時pause維持、`SpeakerRef` に `type` 判別子追加、cue数上限100、日本語エイリアスv1制限明記、`@scene` メタデータ導入。 |
| 1.2 | 2026-04-24 | 実装反映。Emoji Annotation（shortcode 展開 39 種）を v1 正式機能として追加。PCM/WAV コンテンツネゴシエーション（`Accept` ヘッダ）を出力形式として規定。Caption（VoiceDesign）を「API 未対応」から対応済みに更新。`/synth/vds` テキストファイルアップロードエンドポイントを追記。実行時エラーに HTTP ステータスコードを明記。 |
