...
v1 の出力形式は **ストリーミング再生** とする。cue ごとに合成結果の WAV を逐次 `enqueueAudio` に投入し、クライアント側の再生キューで順序制御する。全 cue の合成完了を待たずに再生を開始できるため、レイテンシが低い。

- 各 cue の合成結果は個別の WAV（PCM）としてキューに入る。
- `(pause)` および gap は、指定秒数分の無音 PCM を生成してキューに挿入する。
- 単一結合 WAV へのエクスポートは将来拡張（§8）とし、v1 では扱わない。
...
- **caption エイリアスを含む cue を、caption 未対応のサーバに投入した場合** → 該当 cue をスキップし、ログ／ユーザー通知で「サーバが caption 非対応」である旨を提示。前後の pause は維持。（API 対応が入り次第、この分岐は通常合成に置き換わる。）
...
- **単一結合 WAV エクスポート**: 全 cue を結合した単一 WAV ファイルの出力。ダウンロード・保存用途で有用だが、v1 はストリーミング再生のみ。
...
- **絵文字アノテーションの shortcode 展開**: 上流 Irodori-TTS は[絵文字によるスタイル・効果制御](https://huggingface.co/Aratako/Irodori-TTS-500M/blob/main/EMOJI_ANNOTATIONS.md)（whisper, gasp, joyful 等 39 種）をサポートする。VDS では絵文字を直接書く代わりに `{shortcode}` 形式のインライン記法でテキスト中に埋め込み、パーサが対応する絵文字に展開してから合成に渡す。マッピングを built-in にするかディレクティブ宣言にするかは v2 設計時に決定する。
  ```vds
  chieri: {whisper}ねえ、聞こえる？{gasp}え、何それ？
  # → パーサが展開: 👂ねえ、聞こえる？😮え、何それ？
  ```
  **Shortcode 一覧（上流 EMOJI_ANNOTATIONS.md 準拠）:**
  | Shortcode | 絵文字 | 意味 |
  |-----------|--------|------|
  | `{angry}` | 😠 | 怒って、不機嫌に |
  | `{anxious}` | 😟 | 不安そうに、心配げに |
