...
**Confirm with the user before running step 2**: whether they want a speaker column in the manifest so that reference conditioning actually trains, and which column to key it on. What the code does settle is that a single constant `speaker_id` across a single-speaker dataset would put every clip in one group and engage the concat path. What the code cannot settle is whether that produces a better adapter than training with the speaker branch inert — that is an empirical question about output quality. Do not assert either way; put it to the user.
