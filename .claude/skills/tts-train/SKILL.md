...
The `irodori-tts-train:v4.1` image installs the project into the container's system Python on start — `UV_PROJECT_ENVIRONMENT=/usr` in `docker/train/Dockerfile`, so `uv sync` writes to system site-packages and there is no virtualenv at all. The container is already the isolation boundary, so a venv only adds a directory for a repo bind mount to shadow or for a throwaway `uv run` on the same node to delete (trap 4b below, which the old `/app/.venv`-on-a-named-volume setup invited). Mount `/root/.cache/uv` to keep the start fast. Build on each node from the NFS checkout: `docker build -f docker/train/Dockerfile -t irodori-tts-train:v4.1 .` (Dockerfile.dockerignore keeps data/outputs*/models out of the context). A warning about the environment "not satisfying Python 3.10" is expected and harmless: the image runs 3.12 while the bind-mounted checkout's `.python-version` requests 3.10 (`requires-python` is `>=3.10`, so the env is valid). Note also `wandb sync` cannot reach the CF-protected server — the CF-Access headers ride only through `wandb.Settings(x_extra_http_headers=...)` in train.py's init path, which the sync CLI does not use; an offline run that must be on W&B is cheaper to just re-train online.
...
4b. **`uv run` in a fresh container DELETES and recreates the project venv** — even with `--no-sync`. Harmless now that each container owns its `/opt/venv`, but fatal whenever a venv is shared through a named volume: a throwaway `docker run ... uv run --no-sync python ...` for a quick check wiped the venv out from under every running training container on that node, twice in one session. Never mount a venv as a shared volume (the uv cache is fine to share — it is content-addressed). For pure file operations (rm/mv/cat) use `ubuntu:24.04` and mount nothing else. If a venv does get wiped, `docker exec <training-container> uv sync --frozen --no-dev` restores it without a restart — already-running train.py processes survive on their open file handles.
...
Post-campaign export: write `data/<sid>/config.yaml` for every speaker BEFORE exporting — the exporter reads it via the checkpoint's saved manifest path and bakes `speaker.label` (Japanese display name), `speaker.cv` (Japanese VA), `category.id`/`category.label` (`genshin_impact`/原神, `honkai_star_rail`/崩壊スターレイル, `wuthering_waves`/鳴潮) into the safetensors metadata alongside the uuid5 and inference defaults. Label/CV sources: gi.yatta.moe / sr.yatta.moe API (`/api/v2/{jp,en}/avatar`, CV in `fetter.cv.JP` — present for Genshin, absent for HSR, so HSR CVs came from the wikiwiki 参加声優一覧 page; WuWa and NPCs from JP wiki pages). Batch export picks each speaker's lowest-val `checkpoint_best_val_loss_*` (fallback `checkpoint_final`) and is incremental — rerunning skips existing outputs in `export/v4.1-small/`. Remember `/tmp` is node-local: a `-v /tmp/x.sh:...` mount on a node where the file was never scp'd silently creates a directory and the container fails with "Is a directory".

Speaker ids must be English from the start — W&B run names are immutable and renaming 183 dirs mid-campaign costs a full stop. The corpora (`ultemica/*-voices`) carry only Chinese speaker names, but official English names are derivable from the data: voice filenames end in `..._<slug>_<idx>` where the slug is the language-independent internal codename, and `simon3000/genshin-voice` / `simon3000/starrail-voice` label every row with the official English speaker name — so slug -> official name needs only 2-3 of their shards (exact-stem joins are too sparse; slug-level voting converges immediately). WuWa filenames name the SCENE (`Character_Brant_...` appears in other speakers' lines), so its 21 speakers were mapped by hand. The applied mapping lives at `maps/rename_en.tsv` on the cluster checkout; per-speaker vote details in that session's `/tmp/rename_map_votes.json`. Judgment calls: `gi_aether`/`gi_lumine` (data says Hero/Heroine), `gi_columbina` vs `gi_the_damselette` (data calls both "The Damselette"), `gi_zijiao` (fandom wiki says Zibai; game data says Zijiao — data won).
