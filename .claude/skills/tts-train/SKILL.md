...
4b. **`uv run` in a fresh container DELETES and recreates `/app/.venv`** — even with `--no-sync`. The named volume `irodori_v4_venv` is shared, so a throwaway `docker run ... uv run --no-sync python ...` for a quick check wipes the venv out from under every long-running training container on that node (this happened twice in one session). In any fresh container, run `uv sync --frozen --no-dev` FIRST and only then `uv run --no-sync ...`; for pure file operations (rm/mv/cat) use `ubuntu:24.04` without mounting the venv volume at all. If the venv does get wiped, `docker exec <training-container> uv sync --frozen --no-dev` restores it without a restart — already-running train.py processes survive on their open file handles.
