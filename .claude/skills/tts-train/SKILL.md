...
3. **write training config** — materialize a per-speaker YAML derived from `configs/train_500m_v2/lora/default.yaml`
...
5. **Config overrides** — present these defaults (from `configs/train_500m_v2/lora/default.yaml`) and ask whether any should change:
   - `--max-steps 5000`
   - `--batch-size 8`
...
Start from `configs/train_500m_v2/lora/default.yaml` and write a derived file `configs/train_500m_v2/lora/<speaker>.yaml` with any user-specified overrides (run name, max_steps, lora params, etc.). Do not modify the base config in place.
...
uv run python train.py \
  --config configs/train_500m_v2/lora/<speaker>.yaml \
